...
Class modelosmodel{
        ...
        public function ActualizarModelo($fabricante, $modelo, $especificaciones, $imagen, $historia, $juegos, $id=' '){
            $query = $this->db->prepare("UPDATE modelos SET modelos_modelo = :modelo, modelos_especificaciones = :espesificaciones, modelos_imagen = :imagen, modelos_historia = :historia, modelos_juegos = :juegos, categoria_id = :fabicante WHERE modelos_id = :id");
        ...
        public function obtenerlosmodelos($sort, $order) {
            $query = $this->db->prepare('SELECT * FROM `modelos` ORDER BY ' . $sort . ' ' . $order);
            $query->execute();
            return $query->fetchAll(PDO::FETCH_OBJ);
        }
    }