...
Class consolamodel{
        ...
        public function ActualizarCategoria($fabricante, $logo, $id=' '){
            $query = $this->db->prepare("UPDATE consolas SET categoria_fabricante = :fabricante, categoria_logo = :logo WHERE categoria_id  = :id");
        ...
        public function ObtenerlasConsolas($sort, $order) {
            $query = $this->db->prepare('SELECT * FROM `consolas` ORDER BY ' . $sort . ' ' . $order);
            $query->execute();
            return $query->fetchAll(PDO::FETCH_OBJ);
        }
}